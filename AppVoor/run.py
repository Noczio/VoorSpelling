...
class DataSetWindow(Window):
    ...
    def _set_last_emitted(self, event: str) -> None:
        self._last_btn_used = event if event is "load" or "drag" else "any"
        btn_load_style = load_buttons_style[self._last_btn_used][0]
        btn_drag_style = load_buttons_style[self._last_btn_used][-1]
...
class Training(Window):
    ...
    def close_window(self) -> None:
        super(Training, self).close_window()
        widget.close()
...
class StepByStepLoad(Training):
    ...
    def _train_model(self) -> None:
        ...
        score = model.score_model(data_frame, score_type[prediction_type], 10)
